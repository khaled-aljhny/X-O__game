...
def check_winner():
    ...
    for row in range(3):
        if game_btns[row][0]['text'] == game_btns[row][1]['text'] == game_btns[row][2]['text'] !="":
            game_btns[row][0].config(bg="#64F504")
            game_btns[row][1].config(bg="#64F504")
            game_btns[row][2].config(bg="#64F504")
            return True
    # check all 3 vertical conditions
    for col in range(3):
        if game_btns[0][col]['text'] == game_btns[1][col]['text'] == game_btns[2][col]['text'] !="":
            game_btns[0][col].config(bg="#64F504")
            game_btns[1][col].config(bg="#64F504")
            game_btns[2][col].config(bg="#64F504")
            return True
        
    # check diagonals conditions 
    if game_btns[0][0]['text'] == game_btns[1][1]['text'] == game_btns[2][2]['text'] !="":
            game_btns[0][0].config(bg="#64F504")
            game_btns[1][1].config(bg="#64F504")
            game_btns[2][2].config(bg="#64F504")
            return True
    elif game_btns[0][2]['text'] == game_btns[1][1]['text'] == game_btns[2][0]['text'] !="":
            game_btns[0][2].config(bg="#64F504")
            game_btns[1][1].config(bg="#64F504")
            game_btns[2][0].config(bg="#64F504")
            return True
